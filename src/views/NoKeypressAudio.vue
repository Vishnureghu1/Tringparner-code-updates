                                ...
                                <div class="ml-8">
                                    ...
                                    <v-row class="mt-0" align="center" justify="center">
                                        ...
                                        <v-col cols="10" sm="4" class="d-flex">
                                            <v-file-input :rules="rules" accept=".mp3, .wav" placeholder="Choose file"
                                                prepend-icon="mdi-music" label="Upload File" show-size ref="myfile"
                                                v-model="file"></v-file-input>
                                            <v-btn :disabled="isActiveUploadBtn" :loading="dialog"
                                                @click="uploadGreetingMessage()" class="ma-2" color="primary" dark>
                                                + Upload Media
                                            </v-btn>
                                            <v-progress-linear color="primary " buffer-value="0" height="35"
                                                :value="uploadedValue" striped v-show="isProgressing">
                                            </v-progress-linear>
                                            <v-progress-linear color="red" buffer-value="0" :value="uploadedValue"
                                                stream v-show="isProgressing"></v-progress-linear>
                                        </v-col>
                                    </v-row>
                                                ...
                                                <v-row no-gutters>
                                                    <v-col cols="12">
                                                        <v-card class="mb-0 mt-0" :elevation="0">
                                                            ...
                                                            <v-card class="ml-5 mt-5 text--red" elevation="0"
                                                                v-if="!radioGroup">No audio selected!</v-card>
                                                            <v-radio-group v-model="radioGroup" :mandatory="false"
                                                                v-on:change="
                                                                    $emit('greeting_message_changed', $event)
                                                                ">
                                                                <v-card color="transparent" outlined class="mt-0 pl-6"
                                                                    max-width="1069" v-for="greeting in Greetings"
                                                                    :key="greeting.id">
                                                                    <v-row align="center" justify="center">
                                                                        <v-col cols="12" sm="4">
                                                                            <div v-if="
                                                                                radioGroup == greeting.AudioAccountId
                                                                            ">
                                                                                <v-radio :ref="radioGroup"
                                                                                    :value="greeting.AudioAccountId"
                                                                                    @click="
                                                                                        triggerUnselectAudio(
                                                                                            greeting.AudioAccountId
                                                                                        )
                                                                                    ">
                                                                                    <span slot="label"
...
export default {
    ...
    created() {
        ...
        this.$on("greeting_message_changed", function (id) {
            ...
            const options = {
                url: this.$cloudfareApi + "/callDistribution/ivr/prompt",
                method: "POST",
                headers: {
            ...
            .then(async (snapshot) => {
                if (!snapshot.empty) {
                    snapshot.docs.forEach((element) => {
                        ...
                        this.radioGroup = element.data().IvrNoinputPrompt;
                    });
                } else {
